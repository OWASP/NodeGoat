...
var zapOptions = {proxy: (sutProtocol + config.zapHostName  + ":" + config.zapPort + "/"), targetApp: zapTargetApp};
...
// Easiest way to understand how this works and go through the steps is to setup uthentication for a user using Zap manually first
...
test.before(function () {
   ...
   webDriver = new seleniumWebdriver.Builder()
      ...
      // Proxy all requests through Zap before using Zap to find vulnerabilities, otherwise Zap will say: "URL not found in the scan tree".
   ...
   webDriver.findElement({tagName: "button", type: "submit"}).click();
   webDriver.sleep(1000);
...
test.describe(zapTargetAppRoute + " regression test suite", function () {
   ...
   test.it("Should not exceed the decided threshold of vulnerabilities known to Zap", function (done) {
      ...
      var onCompletion = function (error, resultsFromAllAsyncSeriesFunctions) {
         if(!error) console.log(resultsFromAllAsyncSeriesFunctions[resultsFromAllAsyncSeriesFunctions.length - 1].description);
         else throw error;
         if (numberOfAlerts > alertThreshold)
            console.log("Search the generated report for \"/" + zapTargetAppRoute + "\" to see the " + (numberOfAlerts - alertThreshold) + " vulnerabilities that exceed the user defined threshhold of: " + alertThreshold);
      ...
      async.series([
         ...
         function setAuthenticationCredentials(setAuthenticationCredentialsDone) {
            zaproxy.users.setAuthenticationCredentials(contextId, userId, "username=" + sutUserName + "password=" + sutUserPassword, function (err, resp) {
                  ...
                  function status() {
                     zaproxy.ascan.status(scanId, function (err, resp) {
                        ...
                        zaproxy.core.numberOfAlerts(zapTargetAppAndRoute, function (err, resp) {
                           ...
                           console.log("Scan " + scanId + " is " + statusValue + "% complet with " + numberOfAlerts + " alerts.");
                        });
                     });                     
                  }
                  zapInProgressIntervalId = setInterval(function () {
                     status();
                     if (zapError && statusValue != 100) {
                        ...
                        console.log("We are finishing scan " + scanId +  ". Please see the report for further details.");
                        clearInterval(zapInProgressIntervalId);
                        status();
                        console.log("About to write report.");
                        zaproxy.core.htmlreport(function (err, resp) {
                           var date = new Date();
                           var reportPath = __dirname + "/report_" + date.getFullYear() + "-" + (date.getMonth()+1) + "-" + date.getDate() + "-" + date.getHours() + "-" + date.getMinutes() + ".html";
